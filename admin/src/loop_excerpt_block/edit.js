...
export default function Edit(props) {
	const { attributes: { show_rsvp_button, hide_excerpt, hide_date }, setAttributes, context, isSelected } = props;
    ...
    class ExcerptInspector extends Component {
            ...
            render() {
                const { attributes: { show_rsvp_button, hide_excerpt, hide_date }, setAttributes, isSelected } = this.props;
                    return (
                        <div>
                    <InspectorControls key="excerptinspector">
                    <PanelBody title={ __( 'RSVPMaker Excerpt', 'rsvpmaker' ) } >
                    <ToggleControl
                label={__("Hide Event Date/Time (shown by default)",'rsvpmaker')}
                checked={ hide_date }
                onChange={ ( hide_date ) => { setAttributes( { hide_date } ) } }
            />
                    <ToggleControl
                label={__("Hide Excerpt (shown by default)",'rsvpmaker')}
