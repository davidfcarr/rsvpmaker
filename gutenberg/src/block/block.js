...
apiFetch( {path: 'rsvpmaker/v1/future'} ).then(
	events => {
    if (Array.isArray( events )) {
         events.map(
        function(event) { if (event.ID) {
                var title = (event.date) ? event.post_title + ' - ' + event.date : event.post_title; rsvpupcoming.push(
                 {value : event.ID,
                        label : title }
                ) } }
        );
...
apiFetch( {path: 'rsvpmaker/v1/types'} ).then(
	types => {
    if (Array.isArray( types )) {
        types.map(
        function(type) { if (type.slug && type.name) {
                rsvptypes.push( {value: type.slug, label: type.name } ) }
        }
        );
    } else {
...
apiFetch( {path: 'rsvpmaker/v1/authors'} ).then(
	authors => {
    if (Array.isArray( authors )) {
        authors.map(
        function(author) { if (author.ID && author.name) {
                rsvpauthors.push( {value: author.ID, label: author.name } ) }
        }
        );
    } else {
