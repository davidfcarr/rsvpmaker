	...
	if ( is_admin() && ( $post->post_type == 'rsvpmaker' ) && isset( $_GET['action'] ) && $_GET['action'] == 'edit' ) {
		...
		if ( $email_templates_array ) {
			foreach ( $email_templates_array as $index => $template ) {
				if ( $index > 0 ) {
					$confirmation_email_templates[] = array(
						'label' => $template['slug'],
						'value' => $index,
					);
				}
		...
		if ( isset( $post->post_type ) && ( $post->post_type == 'rsvpmaker' ) ) {
			wp_localize_script(
				'rsvpmaker_admin_script',
				'rsvpemail_scheduling',
				admin_url( 'edit.php?post_type=rsvpemail&page=rsvpmaker_scheduled_email_list&post_id=' . $post_id )
			);
