...
class CssInliner extends AbstractHtmlProcessor {
	...
	private function extractImportAndCharsetRules( $css ) {
		...
		return array( $possiblyModifiedCss, $importRules );
	}
	...
	private function parseCssRules( $css ) {
		...
		\usort( $cssRules['inlinable'], array( $this, 'sortBySelectorPrecedence' ) );
	...
	private function determineMatchingUninlinableCssRules( array $cssRules ) {
		$this->matchingUninlinableCssRules = \array_filter( $cssRules, array( $this, 'existsMatchForSelectorInCssRule' ) );
	}
	...
	private function removeUnmatchablePseudoComponents( $selector ) {
		...
		$selectorWithoutNots = \ltrim(
			\preg_replace_callback(
				'/(\\s?+):not(\\([^()]*+(?:(?2)[^()]*+)*+\\))/i',
				array( $this, 'replaceUnmatchableNotComponent' ),
				' ' . $selector
			)
		);
		...
		return \preg_replace(
			array( '/(\\s|^)' . $pseudoComponentMatcher . '/i', '/' . $pseudoComponentMatcher . '/i' ),
			array( '$1*', '' ),
			$selectorWithoutNots
		);
	}
	...
	private function copyUninlinableCssToStyleNode( $cssImportRules ) {
		...
		if ( $this->matchingUninlinableCssRules !== array() ) {
